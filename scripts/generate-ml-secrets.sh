...
# Generate combined ML platform secret (MinIO + MLflow)
generate_sealed_secret \
    "${NAMESPACE}-ml-platform" \
    "generic" \
    "$OUTPUT_DIR/${NAMESPACE}-ml-platform-sealed-secret.yaml" \
...
# Generate GHCR secret (separate due to different structure)
generate_sealed_secret \
    "${NAMESPACE}-ghcr" \
    "docker-registry" \
    "$OUTPUT_DIR/${NAMESPACE}-ghcr-sealed-secret.yaml"
...
## Files Included:
- ${NAMESPACE}-ml-platform-sealed-secret.yaml - Combined ML platform credentials (MinIO + MLflow)
- ${NAMESPACE}-ghcr-sealed-secret.yaml - GitHub Container Registry credentials

## Usage:
1. Apply these sealed secrets to your namespace:
   \`\`\`bash
   kubectl apply -f ${NAMESPACE}-ml-platform-sealed-secret.yaml
   kubectl apply -f ${NAMESPACE}-ghcr-sealed-secret.yaml
   \`\`\`

2. Reference in your deployments:
   \`\`\`yaml
   env:
...
tar -czf "$OUTPUT_DIR/${NAMESPACE}-secrets-package.tar.gz" -C "$OUTPUT_DIR" .
echo "📦 Package created: $OUTPUT_DIR/${NAMESPACE}-secrets-package.tar.gz"